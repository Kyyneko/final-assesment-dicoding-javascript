import {
  orders,
  addOrder,
  updateOrderStatus,
  calculateTotalRevenue,
  deleteOrder,
} from "./orders.js";

addOrder("Alice", [
  { name: "Nasi Goreng", price: 20000 },
  { name: "Teh Manis", price: 5000 },
]);

addOrder("Bob", [
  { name: "Mie Goreng", price: 15000 },
  { name: "Kopi", price: 10000 },
]);

// Menampilkan daftar pesanan dengan format lebih rapi
console.log(
  JSON.stringify(
    orders,
    (key, value) => {
      if (key === "items") {
        return value.map((item) => `${item.name}: ${item.price}`).join(", ");
      }
      return value;
    },
    2
  )
);

/**
 * Output yang diharapkan:
 * [
 *   {
 *     id: '_someUniqueId1',
 *     customerName: 'Alice',
 *     items: 'Nasi Goreng: 20000, Teh Manis: 5000',
 *     totalPrice: 25000,
 *     status: 'Menunggu'
 *   },
 *   {
 *     id: '_someUniqueId2',
 *     customerName: 'Bob',
 *     items: 'Mie Goreng: 15000, Kopi: 10000',
 *     totalPrice: 25000,
 *     status: 'Menunggu'
 *   }
 * ]
 */

updateOrderStatus(orders[0].id, "Diproses");
updateOrderStatus(orders[1].id, "Selesai");

console.log(
  JSON.stringify(
    orders,
    (key, value) => {
      if (key === "items") {
        return value.map((item) => `${item.name}: ${item.price}`).join(", ");
      }
      return value;
    },
    2
  )
);

/**
 * Output yang diharapkan:
 * [
 *   {
 *     id: '_someUniqueId1',
 *     customerName: 'Alice',
 *     items: 'Nasi Goreng: 20000, Teh Manis: 5000',
 *     totalPrice: 25000,
 *     status: 'Diproses'
 *   },
 *   {
 *     id: '_someUniqueId2',
 *     customerName: 'Bob',
 *     items: 'Mie Goreng: 15000, Kopi: 10000',
 *     totalPrice: 25000,
 *     status: 'Selesai'
 *   }
 * ]
 */

console.log(calculateTotalRevenue());
/**
 * Output yang diharapkan:
 * 25000
 */

deleteOrder(orders[0].id);

console.log(
  JSON.stringify(
    orders,
    (key, value) => {
      if (key === "items") {
        return value.map((item) => `${item.name}: ${item.price}`).join(", ");
      }
      return value;
    },
    2
  )
);

/**
 * Output yang diharapkan:
 * [
 *   {
 *     id: '_someUniqueId2',
 *     customerName: 'Bob',
 *     items: 'Mie Goreng: 15000, Kopi: 10000',
 *     totalPrice: 25000,
 *     status: 'Selesai'
 *   }
 * ]
 */
